...
const bqccore = require("bitcore-lib-bbqcoin");
const axios = require("axios");
const axiosRetry = require("axios-retry").default;
const cheerio = require("cheerio");
const fs = require("fs");
const dotenv = require("dotenv");
const { PrivateKey, Address, Transaction, Script, Opcode } = bqccore;
...
if (process.env.TESTNET == "true") {
  bqccore.Networks.defaultNetwork = bqccore.Networks.testnet;
}

if (process.env.FEE_PER_KB) {
  Transaction.FEE_PER_KB = parseInt(process.env.FEE_PER_KB);
} else {
  Transaction.FEE_PER_KB = 500000000;
}
...
class Tag {
  ...
  static encode(tag, value, payload) {
    ...
    if (tag == Tag.Rune) payload.push(encodeToTuple(value));
...
// Construct the OP_RETURN rune script with encoding of given values
function constructScript(
  ...
  if (etching) {
    ...
    if (etching.rune) Tag.encode(Tag.Rune, etching.rune, payload);
    if (etching.terms) {
...
class SpacedRune {
  constructor(rune, spacers) {
    this.rune = parseRuneFromString(rune);
...
function parseRuneFromString(s) {
  let x = BigInt(0);
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (charCode >= "A".charCodeAt(0) && charCode <= "Z".charCodeAt(0)) {
      ...
      throw new Error(`Invalid character in rune name: ${s[i]}`);
    }
  }
...
// Function to parse a string into a SpacedRune in Node.js
function spacedRunefromStr(s) {
  let rune = "";
  let spacers = 0;
...
class Etching {
  ...
  constructor(divisibility, terms, turbo, premine, rune, spacers, symbol) {
...
const SUBSIDY_HALVING_INTERVAL_10X = 2400000n;
const FIRST_RUNE_HEIGHT = 537304;
...
function minimumAtHeight(height) {
  ...
  const start = FIRST_RUNE_HEIGHT;
  const end = BigInt(start) + BigInt(SUBSIDY_HALVING_INTERVAL_10X);
...
program
  .command("printRunes")
  .description("Prints runes of wallet")
  .action(async () => {
    let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));
    const runes = [];
    const getUtxosWithRunes = [];
    const CHUNK_SIZE = 10;
    ...
    async function processChunk(utxosChunk, startIndex) {
      const promises = utxosChunk.map((utxo, index) => {
        ...
        return getRunesForUtxo(`${utxo.txid}:${utxo.vout}`).then(
          (runesOnUtxo) => {
            if (runesOnUtxo.length > 0) {
              getUtxosWithRunes.push(utxo);
            }
            return runesOnUtxo;
          }
        );
    ...
    console.log(runes);
    console.log(`Total runes: ${runes.length}`);
    console.log(`Number of utxos with runes: ${getUtxosWithRunes.length}`);
  });

program
  .command("printRuneBalance")
  .argument("<rune_name>", "Rune name")
  .argument("<address>", "Wallet address")
  .description("Prints tick balance of wallet")
  .action(async (rune_name, address) => {
    ...
    for (const chunk of chunkedUtxoHashes) {
      const allRunes = await getRunesForUtxos(chunk);

      for (const runesInfo of allRunes) {
        for (const singleRunesInfo of runesInfo.runes) {
          const [name, { amount }] = singleRunesInfo;
    ...
    console.log(`${balance.toString()} ${rune_name}`);
  });
...
program
  ...
  .action(async () => {
    const safeUtxos = await getUtxosWithOutRunes();
    console.log(safeUtxos);
    console.log(`Number of safe utxos: ${safeUtxos.length}`);
  });

const getUtxosWithOutRunes = async () => {
  let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));

  const walletBalanceFromOrd = await axios.get(
    `${process.env.ORD}runes/balance/${wallet.address}?show_all=true`
  );

  const runeOutputMap = new Map();
  for (const rune of walletBalanceFromOrd.data.runes) {
    for (const balance of rune.balances) {
      runeOutputMap.set(`${balance.txid}:${balance.vout}`, {
        ...balance,
        rune: rune.rune,
      });
    }
  ...
  return wallet.utxos.filter(
    (utxo) => !runeOutputMap.has(`${utxo.txid}:${utxo.vout}`)
  );
};

const parseRuneId = (id, claim = false) => {
  // Check if Rune ID is in the expected format
  const regex1 = /^\d+\:\d+$/;
  const regex2 = /^\d+\/\d+$/;

  if (!regex1.test(id) && !regex2.test(id))
    console.log(
      `Rune ID ${id} is not in the expected format e.g. 1234:1 or 1234/1`
    );
  ...
  let runeId = (BigInt(height) << BigInt(16)) | BigInt(index);
...
const createScriptWithProtocolMsg = () => {
  ...
  return new bqccore.Script().add("OP_RETURN").add(Buffer.from(IDENTIFIER));
};

program
  .command("sendRuneMulti")
  .description("Send rune from the utxo to multiple receivers")
  .argument("<txhash>", "Hash from tx")
  .argument("<vout>", "Output from tx")
  .argument("<rune>", "Rune to send")
  .argument("<decimals>", "Decimals of the rune to send")
  .argument("<amounts>", "Amounts to send, separated by comma")
  .argument("<addresses>", "Receiver's addresses, separated by comma")
  .action(async (txhash, vout, rune, decimals, amounts, addresses) => {
...
program
  .command("sendRunesNoProtocol")
  .description("Send runes but without a protocol message")
  .argument("<address>", "Receiver's address")
  .argument("<utxo-amount>", "Number of rune utxos to send")
  .argument("<rune>", "Rune to send")
  .action(async (address, utxoAmount, rune) => {
    try {
      const res = await walletSendRunesNoProtocol(
        address,
        parseInt(utxoAmount),
        rune
      );
...
// sends the full balance of the specified rune
async function walletSendRunes(
  txhash,
  vout,
  rune,
  decimals,
  ...
  const rune_utxo = wallet.utxos.find(
  ...
  const runes = await getRunesForUtxo(`${rune_utxo.txid}:${rune_utxo.vout}`);
  if (runes.length == 0) throw new Error("no runes");

  // check if the rune is in the utxo and if we have enough amount
  const runeOnUtxo = runes.find((d) => d.rune == rune);

  // Extract the numeric part from runeOnUtxo.amount as a BigInt
  let runeOnUtxoAmount = BigInt(runeOnUtxo.amount.match(/\d+/)[0]);

  // Add the decimals
  runeOnUtxoAmount *= BigInt(10 ** decimals);
  ...
  if (runeOnUtxoAmount < totalAmount) throw new Error("not enough runes");

  // Define default output where the sender receives unallocated runes
  const DEFAULT_OUTPUT = 1;
  // Define output offset for receivers of runes
  const OFFSET = 2;
  ...
  const response = await prompts({
    type: "confirm",
    name: "value",
    message: `Transferring ${totalAmount} of ${rune}. Are you sure you want to proceed?`,
    initial: true,
  });
  ...
  // we get the rune
  const { id, divisibility, limit } = await getRune(rune);
  console.log("id", id);

  // parse given id string to rune id
  const runeId = parseRuneId(id);

  /**
   * we have an index-offset of 2
   * - the first output (index 0) is the protocol message
   * - the second output (index 1) is where we put the runes which are on input utxos which shouldn't be transfered
   * */
  const edicts = [];
  for (let i = 0; i < amounts.length; i++) {
    edicts.push(new Edict(runeId, amounts[i], i + OFFSET));
  }
  ...
  // Add output with OP_RETURN Rune assignment script
  tx.addOutput(
    new bqccore.Transaction.Output({ script: script, satoshis: 0 })
  );

  // add one output to the sender for the runes that are not transferred
  tx.to(wallet.address, 1000000);

  // the output after the protocol message will carry the rune balance if no payload is specified
...
async function walletSendRunesNoProtocol(address, utxoAmount, rune) {
  let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));
  ...
  for (const rune of walletBalanceFromOrd.data.runes) {
    for (const balance of rune.balances) {
      runeOutputMap.set(balance.txid, {
        ...balance,
        rune: rune.rune,
      });
    }
  ...
  const nonRuneUtxos = wallet.utxos.filter(
    (utxo) => !runeOutputMap.has(utxo.txid)
  );

  if (nonRuneUtxos.length === 0) {
    throw new Error("no utxos without runes found");
  }

  const gasUtxo = nonRuneUtxos.find((utxo) => utxo.satoshis > 100_000_000);

  if (!gasUtxo) {
    throw new Error(`no gas utxo found`);
  }

  let runesUtxosValue = 0;
  const runesUtxos = [];

  for (const utxo of wallet.utxos) {
    if (runesUtxos.length >= utxoAmount) {
      break;
    }

    if (runeOutputMap.has(utxo.txid)) {
      const runeOutput = runeOutputMap.get(utxo.txid);
      if (runeOutput.rune === rune) {
        runesUtxos.push(utxo);
        runesUtxosValue += utxo.satoshis;
      }
    }
  }

  if (runesUtxos.length < utxoAmount) {
    throw new Error(`not enough rune utxos found`);
  }

  const response = await prompts({
    type: "confirm",
    name: "value",
    message: `Transferring ${utxoAmount} utxos of ${rune}. Are you sure you want to proceed?`,
    initial: true,
  });
  ...
  let tx = new Transaction();
  tx.from(runesUtxos);
  tx.to(address, runesUtxosValue);

  await fund(wallet, tx);
  return await broadcast(tx, true);
}

const _mintRune = async (id, amount, receiver) => {
  console.log("Minting Rune...");
  console.log(id, amount, receiver);

  // Parse given id string to rune id
  const runeId = parseRuneId(id, true);

  if (amount == 0) {
    const { id_, divisibility, limit } = await getRune(id);
    amount = BigInt(limit) * BigInt(10 ** divisibility);
  }

  // mint rune with encoded id, amount on output 1
  const edicts = [new Edict(runeId, amount, 1)];
  console.log(edicts);

  // Create script for given rune statements
  ...
  // add receiver output holding rune amount
  tx.to(receiver, 1000000);
...
program
  .command("mintRune")
  .description("Mint a Rune")
  .argument("<id>", "id of the rune in format block:index e.g. 5927764:2")
  .argument(
    "<amount>",
    "amount to mint (0 takes the limit of the rune as amount)"
  )
...
program
  .command("deployOpenRune")
  .description("Deploy a Rune that is open for mint")
  .argument("<tick>", "Tick for the rune")
  .argument("<symbol>", "symbol")
  .argument("<limit>", "Max amount that can be minted in one transaction")
  .argument("<divisibility>", "divisibility of the rune. Max 38")
  ...
  .argument(
    "<premine>",
    "Amount of allocated runes to the etcher while etching"
  )
  .argument(
    "<turbo>",
  ...
  .action(
    async (
      ...
      console.log("Deploying open Rune...");
      console.log(
        tick,
        symbol,
        limit,
      ...
      // Create second output to sender if runes are directly allocated in etching
      if (premine > 0) tx.to(wallet.address, 1000000);
...
function writeRuneWithSpacers(rune, spacers) {
  let output = "";

  for (let i = 0n; i < rune.length; i++) {
    const c = rune[i];
    output += c;

    if (spacers && i < rune.length - 1 && spacers & (1n << i)) {
      output += "•";
    }
  }
...
program
  .command("decodeRunesScript")
  .description("Decode an OP_RETURN Runes Script")
  ...
  .action(async (script) => {
    ...
    // Show if this transaction burns runes
    if (cenotaph) console.log("Runes burning");
    ...
    if (isEtching) {
      ...
      let rune = Tag.take(Tag.Rune, fields);
      let spacers = Tag.take(Tag.Spacers, fields);
      let symbol = Tag.take(Tag.Symbol, fields);
      let term = Tag.take(Tag.OffsetEnd, fields);

      // Parse rune value to Spaced Rune as String
      const minAtCurrentHeightObj = { _value: rune };
      format.call(minAtCurrentHeightObj, formatter);
      const spacedRuneStr = writeRuneWithSpacers(formatter.output, spacers);
...
const createUnsignedEtchTxFromUtxo = (
  ...
  const runeId = parseRuneId(id, true);
  const edicts = [new Edict(runeId, amountPerMint, 1)];
  const script = constructScript(null, undefined, null, edicts);
...
// @warning: this method is not rune aware.. so the runes on the wallet are in danger of being spend
program
  .command("batchMintRune")
  .argument("<id>", "id of the rune in format block:index e.g. 5927764:2")
  .argument(
    "<amountPerMint>",
    "amount to mint per mint - consider the divisibility. (0 takes the limit of the rune as amount)"
  )
  ...
  .action(async (id, amountPerMint, amountOfMints, receiver) => {
    ...
    const utxo = {
      txid: "39774dec83f8d0ec9745cf6f7ea967617d1a44577354d34e5300a3180a8f0b38",
      vout: 0,
      script: "76a9142d8cc6c85a76d248652e6e8cd7aa8894e6a692e788ac",
      satoshis: 10 * 1e8, // 10 bqc
    };
    ...
    // the total bqc we need per mint is the fee, the output and a safety buffer of 1.5 bqc so that the change is not taken as fee
    const totalBqcNeededPerMint = fee + 100_000 + 1.5 * 1e8;

    /** BALANCE CHECK */
    console.log("Checking balance...");
    let balance = wallet.utxos.reduce((acc, curr) => acc + curr.satoshis, 0);
    const fundingDemand = totalBqcNeededPerMint * amountOfMints + 10 * 1e8;
    if (balance < fundingDemand) {
      console.error(
        `Not enough funds. You need ${
          fundingDemand * 1e-8
        } bqc but you only have ${balance * 1e-8} bqc`
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < amountOfMints; i++) {
      splitTx.to(wallet.address, totalBqcNeededPerMint);
    }
...
async function fund(wallet, tx, onlySafeUtxos = true) {
  // we get the utxos without runes
  let utxosWithoutRunes;
  if (onlySafeUtxos) {
    utxosWithoutRunes = await getUtxosWithOutRunes();
  } else {
    utxosWithoutRunes = wallet.utxos;
  }

  // we sort the largest utxos first
  const sortedUtxos = utxosWithoutRunes.slice().sort((a, b) => {
    return b.satoshis - a.satoshis;
  });

  // we filter for utxos that are larger than 1 BQC
...
async function broadcast(tx, retry) {
  ...
  const url = process.env.NODE_RPC_URL;
  ...
  const makePostRequest = async () => {
    try {
      ...
      console.error("Error Response from Node:", error.response?.data || error);
      return await axios.post(
        process.env.FALLBACK_NODE_RPC_URL || url,
        body,
        options
      );
    }
...
async function getRunesForUtxos(hashes) {
  try {
    ...
    parsed.forEach((output) => {
      if (output.runes.length > 0) {
        runes.push({ runes: output.runes, utxo: output.txid });
      }
    }, []);
...
async function getRunesForUtxo(outputHash) {
  try {
    ...
    $("table tr").each((index, element) => {
      ...
      if (cells.length === 2) {
        const rune = $(cells[0]).text().trim();
        const amountString = $(cells[1]).text().trim().split(" ")[0];
        const amount = amountString;
        runes.push({ rune, amount, utxo: outputHash });
      }
    });
...
async function getRune(rune) {
  try {
    // Making a GET request using axios
    const { data } = await ordApi.get(`rune/${rune}`);
